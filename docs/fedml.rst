...
`FedML <https://github.com/FedML-AI/FedML>`__.
...
The important thing to know is that the procedure to launch FedML
...
after another in a jupyter notebook with flamby.

FedML has three important arguments: the dataset, the trainer and the
...
and test. We will create it using straightforward FLamby code:
...
We will now instantiate the FedML trainer, which is simply a cllass with
``get/set_model_params`` methods and a ``train`` method doing the local
updates, we'll use FLamby's abstraction: ``DataLoaderWithMemory`` in
order to more closely follow FLamby's specifications:
...
Now we can put everything together: we first instantiate a trainer and
we'll use the args and datasets alongside the ``FedAvgAPI`` abstraction:
...
Install MPI by following instructions
`here <https://www.open-mpi.org/faq/?category=building#easy-build>`__.
...
Then we install `mpi4py <https://mpi4py.readthedocs.io/en/stable/>`__.
