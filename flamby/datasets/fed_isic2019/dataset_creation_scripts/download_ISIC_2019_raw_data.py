...
dest_file_1=os.path.join(parent_script_directory,'ISIC_2019_Training_Input.zip')
dest_file_2=os.path.join(parent_script_directory,'ISIC_2019_Training_Metadata.csv')
dest_file_3=os.path.join(parent_script_directory,'ISIC_2019_Training_GroundTruth.csv')
dest_file_4=os.path.join(parent_script_directory,'ISIC_2019_Training_Metadata_FL.csv')

file1=os.path.join(parent_script_directory,'HAM10000_metadata')

#os.system(f"wget {url_1} --no-check-certificate -O {dest_file_1}")
os.system(f"unzip {dest_file_1} -d {parent_script_directory}")
...
HAM10000_metadata.drop(['age', 'sex', 'localization', 'lesion_id', 'dx', 'dx_type'], axis=1, inplace=True)

# taking out images (from image set, metadata file and ground truth file) where datacenter is not available
for i, row in ISIC_2019_Training_Metadata.iterrows():
    if( pd.isnull(row['lesion_id']) ):
        image=row['image']
        os.system("rm "+parent_script_directory+"/ISIC_2019_training_Input/"+image+".jpg")
        if image != ISIC_2019_Training_GroundTruth['image'][i]:
            print("Mismatch between Metadate and Ground Truth")
...
# checking numbers and saving to csv files
print("Datacenters")
...
DIR=os.path.join(parent_script_directory,"ISIC_2019_training_Input")
print( "Number of images + 2 ", len([name for name in os.listdir(DIR) if os.path.isfile(os.path.join(DIR, name))]) )
