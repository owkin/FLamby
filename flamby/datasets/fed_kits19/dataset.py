...
class FedKiTS19(KiTS19Raw):
    ...
    def __init__(
        ...
        if not pooled:
            ...
            self.images = df2.case_ids.tolist()
            c = 0
            self.images_path = OrderedDict() #overwrite
            for i in self.images:
                print(' For silo '+str(center))
                self.images_path[c] = OrderedDict()
                self.images_path[c]['data_file'] = join(self.dataset_directory, "%s.npz" % i)
                self.images_path[c]['properties_file'] = join(self.dataset_directory, "%s.pkl" % i)
                self.images_path[c]['properties'] = load_pickle(self.images_path[c]['properties_file'])
...
if __name__ == "__main__":
    ...
    local_dataset_lengths = [len(FedKiTS19(train=True, pooled=False, center=i)) for i in
                             range(6)]  # [74, 74, 74, 74, 74, 74]
    # we should have:
    ...
    local_dataset_lengths = [len(FedKiTS19(train=False, pooled=False, center=i)) for i in
                             range(6)]  # [74, 74, 74, 74, 74, 74]
    # we should have:
    print(local_dataset_lengths)
